...
bool detectACPI()
{
    ...
    {
        if (BIT_IS_SET(ecx, 31))
        { // 31 is officialy reserved as 0 but QEMU sets it
            return true;
        }
        else
        {
...
{
    RSDP *rsdpval = NULL;
    for (int i = 0; i < (0x000FFFFF); i += 0x1000)
...
void init_acpi()
{
    ...
    for (uint32_t i = 1; i < ((((SDTHeader *)dsdt)->Length / 0x1000) + 1); i++)
        vmm_map_page((void *)((uintptr_t)dsdt + (0x1000 * i)), (void *)((uintptr_t)dsdt + (0x1000 * i)), 0);
