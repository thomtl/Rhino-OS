...
{
    if (p)
        return &p->m_entries[PAGE_TABLE_INDEX((uint32_t)addr)];
    return 0;
}
...
void vmm_map_page(void *phys, void *virt, uint32_t user)
{
    ...
    if ((*e & RHINO_PTE_PRESENT) != RHINO_PTE_PRESENT)
    {
        ...
        if (user)
            vmm_pd_entry_add_attrib(entry, RHINO_PDE_USER);
    ...
    if (user)
        vmm_pt_entry_add_attrib(pagv, RHINO_PTE_USER);
...
pdirectory *vmm_clone_dir(pdirectory *dir)
{
    ...
    for (uint32_t i = 0; i < 1024; i++)
    {
        ...
        if (vmm_pd_entry_is_present(dir->m_entries[i]))
        {
            ...
            if (vmm_pd_entry_is_user(dir->m_entries[i]))
                vmm_pd_entry_add_attrib(pde, RHINO_PDE_USER);
