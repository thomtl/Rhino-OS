...
void init(char *prg){
  ...
  createTask((void*)PROGRAM_LOAD_ADDRESS, root->regs.eflags, ((uint32_t)dir - KERNEL_VBASE));
...
void create_process(char* prg){
  ...
  // Set Interrupt bit
  createTask((void*)PROGRAM_LOAD_ADDRESS, task_for_pid(0)->regs.eflags, ((uint32_t)dir - (uint32_t)KERNEL_VBASE));
