...
void map_vfs_directory(char *c)
{
    ...
    if (strlen(c) == 1)
        f->device = mount_tree->root; //strcmp(c, "/") == 0
    else
        f->device = e;
...
{
    if (!mount_tree || !path || path[0] != PATH_SEPARATOR)
        return NULL;
...
fs_node_t *kopen_recursive(char *filename, uint32_t flags, uint32_t symlink_depth, char *relative_to)
{
    if (!filename)
    {
        debug_log("[VFS]: kopen_recursive no filename");
        return NULL;
    }
    ...
    if (!node_ptr)
    {
        debug_log("[VFS]: kopen_recursive failed to get mount point\n");
        return NULL;
    }
