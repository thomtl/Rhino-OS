...
{
    uint32_t cr2;
    asm volatile("movl %%cr2, %%eax; movl %%eax, %0;"
                 : "=m"(cr2)::"%eax");
...
void nmi_handler(registers_t *regs)
{
    ...
    if (BIT_IS_SET(statusa, 4))
        PANIC_M("[NMI]: Watchdog timer\n");
    if (BIT_IS_SET(statusb, 6))
        PANIC_M("[NMI]: Bus Failure\n");
    if (BIT_IS_SET(statusb, 7))
        PANIC_M("[NMI]: Memory Failure\n");
