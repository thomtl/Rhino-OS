...
static bool ata_wait_for_primary_interrupt(uint32_t timeout)
{
    ...
    while (timeout--)
    {
        if (ata_primary_interrupt_fired == 1)
            return true;
        msleep(1);
    }
...
static bool ata_wait_for_secondary_interrupt(uint32_t timeout)
{
    ...
    while (timeout--)
    {
        if (ata_secondary_interrupt_fired == 1)
            return true;
        msleep(1);
    }
...
{
    if (dev.secondary)
        return ata_wait_for_secondary_interrupt(timeout);
    else
        return ata_wait_for_primary_interrupt(timeout);
}
...
void ata_init(uint16_t bus, uint8_t device, uint8_t function)
{
    ...
    if (bar0 == 0x0)
        bar0 = 0x1F0; // If not defined set ISA values
    if (bar1 == 0x0)
        bar1 = 0x3f4;
    if (bar2 == 0x0)
        bar2 = 0x170;
...
bool ata_init_device(ata_device *dev)
{
    ...
    else if (((dev->id_high == 0x00) && (dev->id_mid == 0x00)) || ((dev->id_high == 0xc3) && (dev->id_mid == 0x3c)))
        dev->atapi = false;
    else if (((dev->id_high == 0xEB) && (dev->id_mid == 0x14)) || ((dev->id_high == 0x96) && (dev->id_mid == 0x69)))
        dev->atapi = true;
    else
    {
...
bool ata_wait(ata_device dev, uint8_t bit, uint32_t timeout)
{
    ...
    while (timeout)
    {
        ...
        {
            if (BIT_IS_SET(status, ATA_STATUS_ERR) || BIT_IS_SET(status, ATA_STATUS_DF))
                return false;
            if (BIT_IS_SET(status, bit))
                return true;
        }
        msleep(1);
        --timeout;
...
bool ata_identify_16(ata_device dev, uint16_t *buf, bool atapi)
{
    ...
    if (ata_wait_for_interrupt(dev, 100))
    {
        for (uint32_t i = 0; i < 256; i++)
            *info++ = inw(dev.cmd_addr + ATA_DATA);
        return true;
    }
    return false;
}
...
bool ata_identify_32(ata_device dev, uint16_t *buf, bool atapi)
{
    ...
    if (ata_wait_for_interrupt(dev, 100))
    {
        for (uint32_t i = 0; i < 128; i++)
            *info++ = ind(dev.cmd_addr + ATA_DATA);
        return true;
    }
    return false;
}
...
bool ata_read(ata_device dev, uint64_t start_sector, uint64_t sectors, void *buf)
{
    if (!dev.exists)
        return false;
    if (dev.lba_max_sectors < (start_sector + sectors))
        return false;
...
bool ata_read_pio_32(ata_device dev, uint64_t start_sector, uint64_t sectors, void *buf)
{
    if (dev.ext_func)
    {
        ...
        if (ata_wait_for_interrupt(dev, 100))
        {
            ...
            {
                for (uint32_t i = 0; i < 128; i++)
                    *data++ = ind(dev.cmd_addr + ATA_DATA);
                if (sectors && !ata_wait_for_interrupt(dev, 100))
                    return false;
            }
            return true;
        }
...
bool ata_read_pio_16(ata_device dev, uint64_t start_sector, uint64_t sectors, void *buf)
{
    ...
    if (dev.ext_func)
    {
        ...
        if (ata_wait_for_interrupt(dev, 100))
        {
            ...
            {
                for (uint32_t i = 0; i < 256; i++)
                    *data++ = inw(dev.cmd_addr + ATA_DATA);
                if (sectors && !ata_wait_for_interrupt(dev, 100))
                    return false;
            }
            return true;
...
bool ata_write_pio_32(ata_device dev, uint64_t start_sector, uint64_t sectors, void *buf)
{
    if (dev.ext_func)
    {
        ...
        if (ata_wait_for_interrupt(dev, 100))
        {
            ...
            {
                for (uint32_t i = 0; i < 128; i++)
                    outd(dev.cmd_addr + ATA_DATA, *data++);
                if (sectors && !ata_wait_for_interrupt(dev, 100))
                    return false;
            }
            return true;
...
bool ata_write_pio_16(ata_device dev, uint64_t start_sector, uint64_t sectors, void *buf)
{
    if (dev.ext_func)
    {
        ...
        if (ata_wait_for_interrupt(dev, 100))
        {
            ...
            {
                for (uint32_t i = 0; i < 256; i++)
                    outw(dev.cmd_addr + ATA_DATA, *data++);
                if (sectors && !ata_wait_for_interrupt(dev, 100))
                    return false;
            }
            return true;
...
bool atapi_send_packet(ata_device dev, uint8_t *packet, uint8_t *return_buffer, uint64_t return_buffer_len)
{
    ...
    if (ata_wait_for_interrupt(dev, 100))
    { //ata_wait(dev, ATA_STATUS_DRQ, 100)){
        for (uint32_t i = 0; i < dev.packet_bytes; i += 2)
            outw(dev.cmd_addr + ATA_DATA, packet[i] | (packet[i + 1] << 8));
