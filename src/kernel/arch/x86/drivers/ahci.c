...
static bool ahci_bios_os_handoff()
{
    ...
    { // if later than 1.2
        ...
        if (BIT_IS_SET(cap2, AHCI_CAP2_BOH))
        {
            ...
            if (BIT_IS_SET(bohc, AHCI_BOHC_BB))
                msleep(2000);

            bohc = ahci_read(AHCI_BOHC);
            if ((bohc & ((1 << AHCI_BOHC_BB) | (1 << AHCI_BOHC_OOS) | (1 << AHCI_BOHC_BOS))) != (1 << 1))
                return false;
...
void ahci_init(uint16_t bus, uint16_t device, uint8_t function)
{
    ...
    if (BIT_IS_CLEAR(ahci_read(AHCI_CAP), AHCI_CAP_SAM))
        ahci_write(AHCI_GHC, (1ULL << AHCI_GHC_EA));
    ...
    for (uint8_t i = 0; i < 32; i++)
    {
        if (BIT_IS_SET(ahci_ports_implemented, i))
        {
            ...
            if (signature == AHCI_SIGNATURE_SATAPI)
                ahci_devices[i].atapi = true;
            else if (signature == AHCI_SIGNATURE_SATA)
                ahci_devices[i].atapi = false;
            else
            {
